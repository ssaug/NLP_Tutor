# functions.py: Script to create the necessary functions for ChatPDF
...
### Function to extract the content of a PDF document
...
### ✅ Updated Function to create a vector of embeddings from the text chunks
def get_vectorstore(chunks):
    if not chunks or not all(isinstance(c, str) and c.strip() for c in chunks):
        raise ValueError(f"Invalid chunks passed to embedding: {chunks}")
    ...
    # Optional: print one encoding test
    print("Example embedding:", embeddings.embed_documents(["hello world"]))

    vectorstore = FAISS.from_texts(texts=chunks, embedding=embeddings)
    return vectorstore


### Function to chain the conversation with the chatbot
